$(document).ready(function(e) {
	...
	queryHTML += '<option value="0"></option><option value="SELECT">SELECT</option><option value="UPDATE">UPDATE</option></select>';
	queryHTML += '<span class="queryHTMLValues"></span></p>';
	/*queryHTML += '<span> x <input type="text" class="numvalue"/>';
	queryHTML += ' y <input type="text" class="numvalue"/>';
	queryHTML += ' z <input type="text" class="numvalue"/></span>';
	queryHTML += ' value <input type="text" class="numvalue"/></p>';*/
	...
	availableQueries = [];
	var objQueryU = new Query("UPDATE");
	objQueryU.values.push(new QueryValue('x', '')); 
	objQueryU.values.push(new QueryValue('y', '')); 
	objQueryU.values.push(new QueryValue('z', ''));
	objQueryU.values.push(new QueryValue('value', ''));
	availableQueries.push(objQueryU);
	
	var objQueryS = new Query("SELECT");
	objQueryS.values.push(new QueryValue('x1', '')); 
	objQueryS.values.push(new QueryValue('y1', '')); 
	objQueryS.values.push(new QueryValue('z1', ''));
	objQueryS.values.push(new QueryValue('x2', ''));
	objQueryS.values.push(new QueryValue('y2', ''));
	objQueryS.values.push(new QueryValue('z2', ''));
	availableQueries.push(objQueryS);	
